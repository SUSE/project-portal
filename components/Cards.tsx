...
export const FeaturedProjectCard = ({ color, logo, tag, name, description, href, repositoryURL, documentationURL }) => {
  return <motion.div initial='hidden'
    animate='visible' variants={motionListItems} className="relative grid  gap-4 px-4 py-4 align-middle ring-2 ring-inset ring-gray-100">
    ...
    <div className="flex flex-col gap-4 w-full">
      <div className="flex items-center gap-2">
        <span className="w-8 h-8">
          <img src={`/project-portal/${logo}`} width="32px" height="32px" alt={`${name} logo`} />
        </span>
        <span className="text-lg font-bold">{name}</span>
      </div>
      <p className="text-xs font-bold text-gray-400">{description}</p>
    </div>
    <div className="flex flex-row justify-between self-baseline place-items-end">
