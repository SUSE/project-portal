import { CATEGORIES, PROJECTS } from '@/db/projects'
...
export const Filter = () => {
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <span className='text-xl text-black font-medium'>Filters</span>
      <p className='text-xs'>
        Categories <span className='text-xs text-gray-400'>(allows multiple choice)</span>
      </p>

      <div className='flex flex-col gap-2'>
        {CATEGORIES.map((category, i) => {
          return (
            <span key={i} className='flex content-start items-start gap-2'>
              <input onChange={handleFilterCategories} className='self-center' type='checkbox' name={category} id={category} />
              <label className='capitalize font-medium' htmlFor={category}>
                {category}
              </label>
            </span>
          )
